...
$\sigma_{\mathrm{sh}}$ is a deterministic angular feature. The implemented vector is
\begin{equation}
\begin{aligned}
...
Routing follows the ITU standard topology and transmitter-height partitions
\cite{iturp1410,saboor2024geometry}. It combines a topology class obtained from
the building-height raster with a class obtained directly from the transmitter
height. Let $\mathcal B$ be the set of building pixels and let its
four-connected components represent the $N_b$ building footprints. With total raster area
$A=513^2\times10^{-6}$~\si{km^2} and maximum raster height $H_j$ in footprint
$j$, we estimate
\begin{equation}
...
mode of the footprint heights. For each ITU prototype $k$, the
selector computes the equal-weight distance
$d_k=\|\log[(\hat\alpha,\hat\beta,\hat\gamma)/
(\alpha_k,\beta_k,\gamma_k)]\|_2$ and chooses the smallest. The componentwise
log-ratios compare relative deviations and prevent the numerical scale of
$\beta$ from dominating. The prototypes are suburban $(0.1,750,8)$, urban
$(0.3,500,15)$, dense urban $(0.5,300,20)$, and urban high rise
$(0.5,300,50)$. Urban high rise is merged
into dense urban because only 90 training maps select that prototype. This
leaves three topology classes. Independently, transmitter height is assigned
to low ($h_{\mathrm{tx}}\le\SI{60}{m}$), middle
($\SI{60}{m}<h_{\mathrm{tx}}\le\SI{100}{m}$), or high
($h_{\mathrm{tx}}>\SI{100}{m}$). The topology and height labels jointly select
one of $3\times3=9$ fitted coefficient sets, without using the city name or
channel target.

For each training map, at most 1024 NLoS pixels are selected deterministically.
Non-intercept features are standardized within regime $q$ and fitted by a ridge regression:
\begin{equation}
...
Building pixels are excluded before receiver statistics or metrics are
computed. The final test metrics therefore contain $423\,087\,119$ valid
non-building receivers: $391\,721\,805$ LoS and $31\,365\,314$ NLoS. NLoS
receivers account for 7.413\% of the evaluated pixels. Although the
...
\SI{1.7412}{dB}. In NLoS, training-only global calibration reduces the raw
...
A compact model retaining eight of the 14 features preserves over 90\% of the
...
This neural result is included only as a preview. A following paper will study
the hybrid approach in depth, using the calibrated attenuation, delay spread,
and angular spread priors both as model inputs and as base estimates for neural
refinement.
