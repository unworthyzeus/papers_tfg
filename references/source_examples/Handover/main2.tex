...
\begin{figure*}[!t]
\centerline{\includegraphics[width=0.9\linewidth]{scenario_builtup.png}} %scenario_builtup/SimulatorTopGC
...
The rest of the paper is organized as follows: Section~II presents the system model and simulation setup. Section~III details the proposed CASH. Section~IV discusses the simulation results and provides key takeaways. Finally, Section~V concludes the paper.
...
This method employs a Fuzzy Logic Controller (FLC) to dynamically tune $\Delta$ and TTT based on $v_u$, $\mathrm{R}_{b_s}(t)$, and estimated GBS load $\lambda$. These inputs are fuzzified into linguistic categories (``slow,'' ``weak,'' ``high load'') and processed through a fuzzy inference engine. The $\Delta$ is obtained as:
...
The proposed CASH protocol is the evolution of A3T, which uses the same handover triggering criteria given in \eqref{a3teq}. However, it also predicts the most suitable neighbouring GBS $b_{c}$ ahead of the UAV ($x_i > x_{\text{UAV}}$) by analyzing its forward trajectory and selecting candidates with sufficient RSRP, as given in Algorithm 1. From this filtered set, it selects the BS with the highest geometric score, calculated by \eqref{cashscore}.
...
higher outage observed for CASH at lower GBS density (e.g., 2/km$^2$) in Fig. \ref{3Dview}. It illustrates how tuning the safety margin $\delta$ in CASH affects performance. With the increase in $\delta$, CASH adopts a more conservative approach by initiating handovers earlier, resulting in more handovers but fewer outages and vice versa. However, depending on the GBS density and underlying urban environment, we can determine the HSM $\delta$ to optimize outages and handovers.
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=0.8\linewidth]{3Dfig.eps}
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=0.8\linewidth]{HSMfig.eps}
