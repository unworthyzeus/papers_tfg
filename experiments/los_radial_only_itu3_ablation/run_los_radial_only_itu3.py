"""Run an isolated FSPL plus refitted radial LoS ablation with ITU routing.

The split helper is the Try 74/75 compatible city-holdout contract used by
Try 80. The LoS radial residual r(d2D, hTx) is refitted from path_loss minus
FSPL on training cities only. No rho, phi, coherent two-ray correction, or
two-ray bias is loaded or evaluated. The NLoS coefficients are frozen from
the completed ITU 3 routing ablation so the LoS change is isolated.

This file is intentionally copied into a separate experiment directory. It
writes only below this directory and does not alter official artifacts or the
earlier ITU 3 experiment.
"""
...
DEFAULT_ITU3_RESULTS = (
    EXPERIMENT_ROOT.parent / "itu_topology_ablation" / "results" / "itu3"
)
DEFAULT_NLOS_REGIME_CALIBRATION = (
    DEFAULT_ITU3_RESULTS / "nlos_regime_calibration_itu.json"
)
DEFAULT_NLOS_GLOBAL_CALIBRATION = (
    DEFAULT_ITU3_RESULTS / "nlos_global_calibration_official.json"
)
DEFAULT_OUTPUT = EXPERIMENT_ROOT / "results" / "radial_only_itu3"
FINAL_FEATURES = tuple(range(1, 15))
GLOBAL_KEY = "all|NLoS|all_ant"
VARIANT_ORDER = (
    "fspl_raw_nlos",
    "radial_fspl_raw_nlos",
    "global_nlos_ridge",
    "regime_nlos_ridge",
)
VARIANT_LABELS = {
    "fspl_raw_nlos": "FSPL LoS + raw COST231 NLoS",
    "radial_fspl_raw_nlos": "FSPL + refitted radial LoS + raw COST231 NLoS",
    "global_nlos_ridge": "FSPL + refitted radial LoS + frozen global NLoS ridge",
    "regime_nlos_ridge": "FSPL + refitted radial LoS + frozen ITU 3 NLoS ridge",
}
HEIGHT_BINS = (
    (12.0, 50.0, "12--50"),
    (50.0, 150.0, "50--150"),
...
def build_routing_audit(
    hdf5_path: Path,
    split_refs: Mapping[str, Sequence[object]],
    ...
    """Classify every map layout and report support in every split."""

    rows: List[Dict[str, object]] = []
    sample_routes: Dict[Tuple[str, str], str] = {}
    ...
    with h5py.File(str(hdf5_path), "r") as handle:
        for split_name, refs in split_refs.items():
            routed_city_counts: Counter[str] = Counter()
            routed_map_counts: Counter[str] = Counter()
            raw_city_counts: Counter[str] = Counter()
            raw_map_counts: Counter[str] = Counter()
            routed_cities: Dict[str, set[str]] = defaultdict(set)
            raw_cities: Dict[str, set[str]] = defaultdict(set)
            all_cities: set[str] = set()
            for ref in refs:
                city = str(ref.city)
                sample_name = str(ref.sample)
                all_cities.add(city)
                topology = np.asarray(
                    handle[ref.city][ref.sample]["topology_map"][...],
                    dtype=np.float32,
                )
                morphology = router.describe(topology)
                sample_routes[(city, sample_name)] = morphology.routed_environment
                record: Dict[str, object] = {
                    "split": split_name,
                    "city": city,
                    "sample": sample_name,
                    "uav_height_m": float(ref.uav_height_m),
                    **morphology.to_dict(),
                }
                rows.append(record)
                routed_map_counts[morphology.routed_environment] += 1
                raw_map_counts[morphology.raw_environment] += 1
                routed_cities[morphology.routed_environment].add(city)
                raw_cities[morphology.raw_environment].add(city)

            routed_city_counts.update(
                {name: len(cities) for name, cities in routed_cities.items()}
            )
            raw_city_counts.update(
                {name: len(cities) for name, cities in raw_cities.items()}
            )
            summary["splits"][split_name] = {
                "cities": len(all_cities),
                "maps": len(refs),
                "cities_with_routed_environment": dict(
                    sorted(routed_city_counts.items())
                ),
                "routed_map_counts": dict(sorted(routed_map_counts.items())),
                "cities_with_raw_environment": dict(sorted(raw_city_counts.items())),
                "raw_map_counts": dict(sorted(raw_map_counts.items())),
            }
    summary["unique_topology_rasters"] = router.cached_topologies
    return rows, summary, sample_routes


def routing_regime_support(
    refs: Sequence[object],
    sample_routes: Mapping[Tuple[str, str], str],
    hybrid_ref,
) -> set[str]:
    return {
        hybrid_ref.regime_key(
            sample_routes[(str(ref.city), str(ref.sample))],
            "NLoS",
            hybrid_ref.ant_bin(ref.uav_height_m),
        )
        for ref in refs
    }
...
def _load_frozen_nlos_calibration(
    path: Path,
    ...
    if feature_names != list(expected_feature_names):
        raise RuntimeError(
            f"feature mismatch in frozen NLoS calibration {path}: "
            f"{feature_names} != {list(expected_feature_names)}"
        )
    coefficients = {
...
def _save_radial_calibration(
    ...
    *,
    ...
    _write_json(
        path,
        {
            "model_type": "fspl_plus_radial_residual_no_two_ray",
            "formula": "PL_LoS = FSPL + r(d2D, hTx)",
            "residual_target": "path_loss - FSPL on valid LoS ground pixels",
            "excluded_parameters": ["rho", "phi", "two_ray_bias"],
            "height_bin_m": height_bin_m,
            "height_bins_m": calibration["height_bins_m"].tolist(),
            "global_profile_db": calibration["global_profile_db"].tolist(),
            "global_count": calibration["global_count"].tolist(),
            "radial_profile_db": calibration["radial_profile_db"].tolist(),
            "radial_profile_smooth_db": calibration[
                "radial_profile_smooth_db"
            ].tolist(),
            "radial_count": calibration["radial_count"].tolist(),
            "fit_contract": {
                "training_maps": training_maps,
                "split_seed": split_seed,
                "membership": "official training cities only",
                "radius_definition": "rounded 1 m d2D rings from the Tx centre",
                "height_interpolation": "linear between 5 m height-bin profiles",
                "residual_clip_db": None,
            },
        },
    )
...
def predict_variants(
    ...
    radial_los = los_model.predict_radial_map(h_tx, radial_calibration)
    ...
    predictions = {
        ...
        "radial_fspl_raw_nlos": np.where(los_flag, radial_los, prior).astype(
            np.float32
        ),
        "global_nlos_ridge": np.where(los_flag, radial_los, global_nlos).astype(
            np.float32
        ),
        "regime_nlos_ridge": np.where(los_flag, radial_los, regime_nlos).astype(
            np.float32
        ),
    }
    return predictions, {
    ...
    *,
    ...
    with h5py.File(str(hdf5_path), "r") as handle:
        for ref in picked:
            ...
            los = los_model.predict_radial_map(ref.uav_height_m, radial_calibration)
    ...
    return {
        ...
        "scope": "HDF5 read measured separately; core computes FSPL plus refitted radial LoS and frozen ITU 3 COST231 ridge NLoS",
...
def main() -> None:
    ...
    parser.add_argument(
        "--nlos-regime-calibration",
        type=Path,
        default=DEFAULT_NLOS_REGIME_CALIBRATION,
    )
    parser.add_argument(
        "--nlos-global-calibration",
        type=Path,
        default=DEFAULT_NLOS_GLOBAL_CALIBRATION,
    )
    parser.add_argument("--out-dir", type=Path, default=None)
    parser.add_argument("--routing", choices=("itu3",), default="itu3")
    parser.add_argument("--component-connectivity", type=int, choices=(4, 8), default=4)
    parser.add_argument("--min-component-area-m2", type=float, default=1.0)
    ...
    VARIANT_LABELS["regime_nlos_ridge"] = (
        f"FSPL + refitted radial LoS + frozen {args.routing} NLoS ridge"
    )
    ...
    routing_rows, routing_summary, sample_routes = build_routing_audit(
        args.hdf5,
    ...
    evaluation_regimes = routing_regime_support(
        list(val_refs) + list(test_refs), sample_routes, hybrid_ref
    )
    print(json.dumps(routing_summary, indent=2, allow_nan=False), flush=True)

    print(
        f"fit LoS radial residual from FSPL [{len(train_refs)} training maps, "
        f"{args.height_bin_m:g} m height bins]",
        flush=True,
    )
    ...
    _save_radial_calibration(
        args.out_dir / "los_radial_only_calibration.json",
        radial_calibration,
        height_bin_m=args.height_bin_m,
    ...
    feature_names = final_feature_names(hybrid_ref)
    regime_coefs, regime_payload = _load_frozen_nlos_calibration(
        args.nlos_regime_calibration, feature_names
    )
    global_coefs, global_payload = _load_frozen_nlos_calibration(
        args.nlos_global_calibration, feature_names
    )
    coefs = {
        "regime_nlos_ridge": regime_coefs,
        "global_nlos_ridge": global_coefs,
    }
    missing_regimes = sorted(evaluation_regimes - set(regime_coefs))
    if missing_regimes:
        raise RuntimeError(
            "evaluation contains regimes absent from the frozen ITU 3 NLoS "
            "calibration: " + ", ".join(missing_regimes)
        )
    if GLOBAL_KEY not in global_coefs:
        raise RuntimeError(
            f"frozen global NLoS calibration lacks required key {GLOBAL_KEY}"
        )
    fit_contract = {
        "los": {
            "formula": "PL_LoS = FSPL + r(d2D, hTx)",
            ...
            "residual_target": "path_loss - FSPL on valid LoS ground pixels",
            "excluded_parameters": ["rho", "phi", "two_ray_bias"],
            "topology_conditioning": None,
        },
        "nlos": {
            "status": "frozen to isolate the LoS ablation",
            "regime_source": str(args.nlos_regime_calibration.resolve()),
            "global_source": str(args.nlos_global_calibration.resolve()),
            "regime_model_type": regime_payload["model_type"],
    ...
    summary = {
        ...
        "final_variant": {
            ...
            "selection_reason": (
                "pre-specified removal of rho and phi with r refitted from FSPL "
                "on training cities only; frozen ITU 3 NLoS coefficients"
            ),
            "test_metrics": test_final,
        },
        "topology_routing": routing_summary,
