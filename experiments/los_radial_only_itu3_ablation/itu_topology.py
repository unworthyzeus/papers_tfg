...
def estimate_itu_parameters(
    ...
    *,
    ...
    likelihood scale, which is also the mode, fitted to one mean roof height
    per connected footprint.
    """
    ...
    if not np.any(building_mask):
        return alpha, 0.0, 0.0, 0, map_area_km2, 0.0
    ...
    if component_count:
        roof_heights = np.asarray(
            ndimage.mean(topo, labels=labels, index=kept_ids), dtype=np.float64
        )
        roof_heights = roof_heights[np.isfinite(roof_heights) & (roof_heights > 0.0)]
        gamma = (
            float(math.sqrt(float(np.mean(np.square(roof_heights))) / 2.0))
            if roof_heights.size
            else 0.0
        )
    else:
        gamma = 0.0
    ...
    return alpha, beta, gamma, component_count, map_area_km2, mean_nonzero_height
...
class ITUTopologyRouter:
    ...
    def describe(self, topology: np.ndarray) -> ITUMorphology:
        ...
        alpha, beta, gamma, count, area, mean_height = estimate_itu_parameters(
            topology,
    ...
    def contract(self) -> Dict[str, object]:
        return {
            ...
            "gamma_estimator": (
                "Rayleigh maximum-likelihood scale sqrt(mean(component_mean_height^2)/2)"
            ),
            "meters_per_pixel": self.meters_per_pixel,
        }
