"""Run the isolated ITU building-maximum routing ablation with two-ray LoS.

The split helper is the Try 74/75 compatible city-holdout contract used by
Try 80. Every calibration is refitted on training cities only. The script
...
Gamma is estimated from one maximum raster height per connected building
footprint. This file is a separate copy and does not alter official artifacts
or either earlier ITU experiment.
"""
...
DEFAULT_OUTPUT = EXPERIMENT_ROOT / "results" / "two_ray_itu3_building_max"
...
VARIANT_LABELS = {
    ...
    "regime_nlos_ridge": "full prior, isolated ITU topology regimes",
}
...
def configure_antenna_height_bins(hybrid_ref) -> None:
    """Use the three antenna-height regimes stated in the paper."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--routing", choices=("itu2", "itu3", "itu4"), default="itu3"
    )
    ...
    parser.add_argument("--residual-clip-db", type=float, default=2.5)
    parser.add_argument("--radius-sigma-px", type=float, default=1.5)
    ...
    VARIANT_LABELS["regime_nlos_ridge"] = (
        f"refitted two-ray LoS + {args.routing} building-maximum ITU routing"
    )
    ...
    train_regimes = routing_regime_support(train_refs, sample_routes, hybrid_ref)
    evaluation_regimes = routing_regime_support(
        list(val_refs) + list(test_refs), sample_routes, hybrid_ref
    )
    missing_regimes = sorted(evaluation_regimes - train_regimes)
    if missing_regimes:
        raise RuntimeError(
            "evaluation contains topology/height regimes absent from training: "
            + ", ".join(missing_regimes)
        )
    print(json.dumps(routing_summary, indent=2, allow_nan=False), flush=True)

    print(
        f"refit LoS radial reference [{len(train_refs)} training maps]",
        flush=True,
    )
    ...
    print("refit rho, phi, and two-ray bias from training cities", flush=True)
    ...
    print("refit radial residual on top of two-ray", flush=True)
    two_ray_residual = los_model.fit_two_ray_residual_calibration(
        args.hdf5,
        train_refs,
        two_ray_calibration=two_ray_calibration,
        height_bin_m=args.height_bin_m,
        residual_clip_db=args.residual_clip_db,
        radius_sigma_px=args.radius_sigma_px,
        verbose=True,
    )
    ...
    los_model.save_calibration(
        radial_calibration,
        two_ray_calibration,
        args.out_dir / "los_two_ray_refitted_calibration.json",
        meta={
            "training_maps": len(train_refs),
            "split_seed": args.split_seed,
            "seed": args.seed,
            "membership": "official training cities only",
            "topology_gamma": "Rayleigh MLE from one maximum raster height per connected building",
        },
    )
    ...
    fit_diagnostics["fit_contract"]["topology_routing"] = router.contract()
    fit_diagnostics["fit_contract"]["antenna_height_routing"] = {
        "thresholds_m": list(ANTENNA_HEIGHT_THRESHOLDS_M),
        "rules": routing_summary["antenna_height_routing"],
    }
    fit_diagnostics["fit_contract"]["los_calibration"] = {
        "status": "refitted from scratch on official training cities",
        "height_bin_m": args.height_bin_m,
        "rho_max": args.rho_max,
        "rho_step": args.rho_step,
        "phi_grid_size": args.phi_grid_size,
    ...
    feature_names = final_feature_names(hybrid_ref)
    _save_calibration(
        args.out_dir / "nlos_regime_calibration_itu.json",
        f"regime_nlos_ridge_cost231_{args.routing}",
        feature_names,
    ...
    summary = {
        ...
        "final_variant": {
            ...
            "selection_reason": (
                "pre-specified ITU routing with one maximum raster height per "
                "connected building; LoS and NLoS recalibrated on training only"
            ),
